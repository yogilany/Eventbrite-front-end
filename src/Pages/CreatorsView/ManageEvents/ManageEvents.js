...
</div> : events.length == 0 ? <div className='bg-gray-200 p-12 rounded'><h1 className='text-xl '>You have not created any events yet</h1></div> : <table class="mt-8 mb-8 w-full text-sm text-left text-gray-500 ">
        ...
        <tbody>
            {
                filteredEvents.map((event) => {
                    return <TableRow event={event} setAllData={setAllData} allData={allData} csvExport={csvExport} />
                })
            }
            ...
            <CSVLink  {...csvReport}>  <MainGrayButton  onClick={() => {
handleCSV();
// setCsvExport(true);
