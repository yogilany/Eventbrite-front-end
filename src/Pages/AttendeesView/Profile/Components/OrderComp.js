  ...
  return (
    <div className="MainComp_prof" data-testid={props.data_testid}>
      <div className="Datee_prof">
        <p class="" style={{ color: "red" }}>{new Date(event?.date_and_time?.start_date_time)
            .toUTCString()
            .slice(8, 11)}</p>
        <p  style={{ color: "grey" }}>{new Date(event?.date_and_time?.start_date_time)
            .toUTCString()
            .slice(4, 7)}</p>
      </div>
      <img onError={handleImgError} src={imgSrc} className="Full_Container_img_prof" />
      <div className="eventdata_prof">
        <h3 class=" font-bold text-lg" style={{ paddingBottom: 0 }}>
          {event?.basic_info?.title}
        </h3>
        <div className="detaileddate_prof">{new Date(event?.date_and_time?.start_date_time)
            .toUTCString()
            .slice(0, -7)} </div>
        <div className="detaileddated_prof">
          ...
          <div>
          {new Date(props.order?.created_date)
            .toUTCString()
            .slice(0, -7)}
          </div> 
        </div>
